...
class RandomDataGenerator():
    ...
    def __init__(self):
        ...
        self.noise_bounds  = {'min':0, 'max':255}
    ...
    def add_noise(self, value):
        noise = normal(self.noise['mean'], self.noise['sd'], 1)
        # Correcting value if the noise puts in out of bounds
        new_value = int(value + noise)
        if new_value > self.noise_bounds['max']:
            return self.noise_bounds['max']
        elif new_value < self.noise_bounds['min']:
            return self.noise_bounds['min']
    ...
    def get_random_dataset(self):
        ...
        while (core_ruler <= core_length):
            ...
            while (layer_ruler <= layer_length):
                rgb_row = [layer_colour[i] for i in range(3)]
                print(rgb_row)
