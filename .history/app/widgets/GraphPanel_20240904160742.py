...
class GraphPanel(QWidget):
    ...
    def init_ui(self):
        ...
        graph.axes_left.set_title('Green')
        graph.axes_center.plot(round(100*df['Green']/255,  4), df['Depth'],  color='red')
        ...
        graph.axes_left.set_title('Blue')
        graph.axes_right.plot(round(100*df['Blue']/255,  4), df['Depth'],  color='red')
        graph.axes_right.grid(axis = 'y')
        graph.axes_right.invert_yaxis()
        graph.axes_right.set_xlabel('Intensity (%)')
